...
RELEASE_JAR="$ROOT/surveye_2_0_0.jar"
...
if ! grep -Fq 'd Version: 2.0.0' "$PKG" ||
   ! grep -Fq 'F surveye_2_0_0.jar' "$PKG" ||
   ! grep -Eq '^[fF] surveye[.]ado$' "$PKG" ||
   ! grep -Eq '^[fF] surveye[.]sthlp$' "$PKG" ||
   ! grep -Eq '^[fF] surveye[.]jar$' "$PKG"; then
  echo "FAIL: package manifest does not identify the v2.0.0 release JAR" >&2
  exit 1
fi
...
if ! printf '%s\n' "$manifest" | grep -Fq 'Implementation-Title: SurvEye' ||
   ! printf '%s\n' "$manifest" | grep -Fq 'Implementation-Version: 2.0.0'; then
