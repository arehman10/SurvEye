...
if [ "$(grep -Fc 'BACKGround(string) TYPOGraphy(string) CORNers(string)' "$ADO" || true)" -ne 2 ] ||
   [ "$(grep -Fc 'SHADow(string) MOTion(string) PAGEWidth(integer 0)' "$ADO" || true)" -ne 2 ] ||
   [ "$(grep -Fc '_surveye_apply_appearance' "$ADO" || true)" -lt 3 ]; then
  echo "FAIL: editorial appearance syntax or post-processing hook is missing" >&2
  exit 1
fi
...
# Stata's viewer corrupts SMCL physical lines that run past roughly 244
...
unbalanced=$(awk '{ o = gsub(/{/, "{"); c = gsub(/}/, "}"); if (o != c) n++ } END { print n+0 }' "$HELP")
if [ "$unbalanced" -ne 0 ]; then
  echo "FAIL: $unbalanced help-file lines leave an SMCL directive open across a line break" >&2
  exit 1
fi
