(function(){
  ...
  var state={filters:dictionary(),charts:dictionary(),visible:dictionary(),renderedRevision:dictionary(),chartRevision:0,visibilityRefreshPending:false,mapRows:[],leafletMap:null,leafletPointRenderer:null,mapPointLayer:null,mapBoundaryLayer:null,mapNeedsFit:true,mapRedrawing:false};
  ...
  function effectiveN(cat){
    if(!CONFIG.weighted)return cat.n;
  ...
  function confidenceInterval(cat,count){
    if(!CONFIG.showCI||(CONFIG.weighted&&(CONFIG.weightType||"").toLowerCase()==="iweight")||!cat||cat.total<=0)return null;var n=effectiveN(cat),p=Math.max(0,Math.min(1,count/cat.total));if(!(n>0))return null;
    ...
    return{low:100*Math.max(0,center-half),high:100*Math.min(1,center+half),level:level,approx:!!(CONFIG.weighted&&(CONFIG.weightType||"").toLowerCase()!=="fweight")};
  }
  ...
  function weightedQuantile(points,p){
    ...
    if(!CONFIG.weighted)return quantile(sorted.map(function(point){return point.value;}),p);
  ...
  function weightedMean(points){
    if(!points.length)return null;var total=0,sum=0;points.forEach(function(point){var w=CONFIG.weighted?Math.max(0,point.w):1;total+=w;sum+=point.value*w;});
  ...
  var discreteMedianGuide={id:"discreteMedianGuide",afterDatasetsDraw:function(chart,args,options){
    if(!options||!isFinite(+options.median)||!chart.scales||!chart.scales.x)return;var area=chart.chartArea,x=chart.scales.x.getPixelForValue(+options.median),ctx=chart.ctx,color=css("--chart-coral","#e5552b"),label=tr(CONFIG.weighted?"weightedMedian":"median",CONFIG.weighted?"Weighted median":"Median")+" "+fmt(+options.median,2);ctx.save();ctx.strokeStyle=color;ctx.lineWidth=1.7;ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(x,area.top);ctx.lineTo(x,area.bottom);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=color;ctx.font="700 9px "+Chart.defaults.font.family;ctx.textBaseline="bottom";var width=ctx.measureText(label).width,padding=3,drawX=Math.max(area.left+padding,Math.min(area.right-width-padding,x-width/2));ctx.textAlign="left";ctx.direction=rtl?"rtl":"ltr";ctx.fillText(label,drawX,area.top-4);ctx.textBaseline="top";if(+options.lowTail>0){ctx.textAlign="left";ctx.fillText("\u25c0 "+localNumber(+options.lowTail),area.left,area.top+3);}if(+options.highTail>0){ctx.textAlign="right";ctx.fillText(localNumber(+options.highTail)+" \u25b6",area.right,area.top+3);}ctx.restore();
  }};
  var distributionGuide={id:"distributionGuide",afterDatasetsDraw:function(chart,args,options){
    if(!options||!options.stats)return;var s=options.stats,ctx=chart.ctx,left=chart.chartArea.left,right=chart.chartArea.right,span=s.displayHigh-s.displayLow;if(!(span>0))return;
    function x(value){return left+Math.max(0,Math.min(1,(value-s.displayLow)/span))*(right-left);}var y=chart.chartArea.top-13,boxTop=y-4,boxBottom=y+4;
    ctx.save();ctx.strokeStyle=css("--ink","#16202e");ctx.fillStyle=rgba(css("--chart-purple","#7c5ba6"),.22);ctx.lineWidth=1.2;ctx.beginPath();ctx.moveTo(x(s.lowerWhisker),y);ctx.lineTo(x(s.upperWhisker),y);ctx.moveTo(x(s.lowerWhisker),y-4);ctx.lineTo(x(s.lowerWhisker),y+4);ctx.moveTo(x(s.upperWhisker),y-4);ctx.lineTo(x(s.upperWhisker),y+4);ctx.stroke();ctx.fillRect(x(s.q1),boxTop,Math.max(1,x(s.q3)-x(s.q1)),boxBottom-boxTop);ctx.strokeRect(x(s.q1),boxTop,Math.max(1,x(s.q3)-x(s.q1)),boxBottom-boxTop);
    var medianX=x(s.median),guideColor=css("--chart-coral","#e5552b");ctx.strokeStyle=guideColor;ctx.lineWidth=1.7;ctx.beginPath();ctx.moveTo(medianX,boxTop-1);ctx.lineTo(medianX,boxBottom+1);ctx.stroke();ctx.setLineDash([5,4]);ctx.beginPath();ctx.moveTo(medianX,chart.chartArea.top);ctx.lineTo(medianX,chart.chartArea.bottom);ctx.stroke();ctx.setLineDash([]);ctx.fillStyle=guideColor;ctx.font="700 8px "+Chart.defaults.font.family;ctx.textBaseline="middle";if(s.lowOutliers){ctx.textAlign="left";ctx.fillText("◀ "+s.lowOutliers,left,y-9);}if(s.highOutliers){ctx.textAlign="right";ctx.fillText(s.highOutliers+" ▶",right,y-9);}ctx.restore();
  }};
  ...
  function buildYesNo(canvas,variable){
    ...
    if(!cat.n||(CONFIG.weighted&&cat.total<=0)){var message=cat.n?tr("noPositiveWeight","No positive weight for this selection"):tr("noValidResponses","No valid responses for this selection");setEmpty(canvas,true,message);statistic(variable,"");clearSummary(variable,message);describe(canvas,message);return;}
  ...
  function buildBar(canvas,variable,multi){
    ...
    if(!cat.n||!cat.keys.length||(CONFIG.weighted&&cat.total<=0)){var message=cat.n?tr("noPositiveWeight","No positive weight for this selection"):tr("noValidResponses","No valid responses for this selection");setEmpty(canvas,true,message);statistic(variable,"");clearSummary(variable,message);describe(canvas,message);return;}
  ...
  function buildFamily(canvas){
    ...
    var anyValid=false;members.forEach(function(member){var cat=categorical(member,source,false);categories[member]=cat;if(cat.n&&(!CONFIG.weighted||cat.total>0))anyValid=true;});
    ...
    var message=details.map(function(item){return item.memberLabel+" ["+item.member+"] — "+item.category+" "+pct(item.share)+"; "+tr("validN","valid n")+"="+localNumber(item.n)+(CONFIG.weighted?"; "+tr("weightedBase","weighted base")+"="+fmt(item.total,1):"");}).join("; ");clearSummary(key,message);describe(canvas,message);canvas.dataset.memberCount=""+members.length;
  ...
  function numericValues(variable,source){
    ...
    for(var i=0;i<source.length;i++){
      ...
      values.push({value:number,w:weight(source[i])});
    }
    return values;
  }
  function weightedStd(points,mean,totalWeight){
    if(points.length<2||mean===null)return null;var sum=0,positive=0;points.forEach(function(point){var w=CONFIG.weighted?Math.max(0,point.w):1,d=point.value-mean;if(w>0)positive++;sum+=w*d*d;});
    var type=(CONFIG.weightType||"").toLowerCase(),variance;
    if(!CONFIG.weighted)variance=sum/(points.length-1);
  ...
  function weightedOutlierShare(stats){return stats&&stats.classified&&stats.totalWeight>0?100*stats.outlierWeight/stats.totalWeight:null;}
  function renderNumericStats(variable,stats){
    ...
    var rows=[[tr("validRawN","Valid raw n"),localNumber(stats.n),tr("missingExcluded","Missing/excluded"),localNumber(stats.missing)],[tr("mean","Mean"),fmt(stats.mean,2),tr("stdDev","Std. dev."),fmt(stats.sd,2)],[tr("minimum","Minimum"),fmt(stats.min,2),tr("maximum","Maximum"),fmt(stats.max,2)],["P25",fmt(stats.q1,2),"P75",fmt(stats.q3,2)],[tr("median","Median"),fmt(stats.median,2),tr("tukeyOutliers","Tukey outliers"),stats.classified?localNumber(stats.outliers):"—"]];
    var outlierShare=weightedOutlierShare(stats);if(CONFIG.weighted)rows.push([tr("weightedOutlierMass","Weighted outlier mass"),stats.classified?fmt(stats.outlierWeight,2):"—",tr("weightedOutlierShare","Weighted outlier share"),outlierShare===null?"—":pct(outlierShare)]);
    ...
    var note=document.createElement("div");note.className="stats-note";var bits=[];if(stats.classified)bits.push(tr("tukeyFences","Tukey fences")+" "+fmt(stats.lowerFence,2)+" "+tr("to","to")+" "+fmt(stats.upperFence,2));else bits.push(tr("outliersNeedFour","Outliers need at least four valid values"));if(CONFIG.weighted)bits.push(tr("weightedSum","weighted sum")+" "+fmt(stats.totalWeight,1),tr("descriptiveWeighted","descriptive weighted statistics"));note.textContent=bits.join(" · ");root.appendChild(note);
  }
  function numericStatistics(points,sourceLength){
    if(!points.length)return null;var totalWeight=points.reduce(function(sum,point){return sum+point.w;},0);if(CONFIG.weighted&&totalWeight<=0)return null;
    var sorted=points.map(function(point){return point.value;}).sort(function(a,b){return a-b;}),mean=weightedMean(points),q1=weightedQuantile(points,.25),medianValue=weightedQuantile(points,.5),q3=weightedQuantile(points,.75),sd=weightedStd(points,mean,totalWeight),iqr=q3-q1,classified=points.length>=4,lowerFence=classified?q1-1.5*iqr:null,upperFence=classified?q3+1.5*iqr:null,outliers=0,outlierWeight=0;
    points.forEach(function(point){if(classified&&(point.value<lowerFence||point.value>upperFence)){outliers++;outlierWeight+=point.w;}});
    return{n:points.length,missing:Math.max(0,sourceLength-points.length),mean:mean,sd:sd,min:sorted[0],max:sorted[sorted.length-1],q1:q1,q3:q3,median:medianValue,classified:classified,lowerFence:lowerFence,upperFence:upperFence,outliers:outliers,totalWeight:totalWeight,outlierWeight:outlierWeight};
  }
  ...
  function discreteDistributionPlan(points,stats,compactMode){
    if(!points||!points.length||!stats)return null;var rounded=points.map(function(point){return{value:Math.round(point.value),w:point.w};}),observed=rounded.map(function(point){return point.value;}),observedMin=Math.min.apply(null,observed),observedMax=Math.max.apply(null,observed),exactLimit=compactMode?36:60,fullSupport=observedMax-observedMin+1,inliers=[];
    rounded.forEach(function(point){if(!stats.classified||(point.value>=stats.lowerFence&&point.value<=stats.upperFence))inliers.push(point);});if(!inliers.length)inliers=rounded.slice();
    var inlierValues=inliers.map(function(point){return point.value;}),robustMin=Math.min.apply(null,inlierValues),robustMax=Math.max.apply(null,inlierValues),trimmed=fullSupport>exactLimit,displayMin=trimmed?robustMin:observedMin,displayMax=trimmed?robustMax:observedMax,support=displayMax-displayMin+1,maxBins=compactMode?36:56,binWidth=support<=exactLimit?1:niceNumericStep(support/maxBins,true,false);
    ...
    var binCount=Math.max(1,Math.round(support/binWidth)),bins=[],weighted=new Array(binCount).fill(0),raw=new Array(binCount).fill(0),outlierRaw=new Array(binCount).fill(0),lowTail=0,highTail=0,lowTailWeight=0,highTailWeight=0;
    for(var b=0;b<binCount;b++){var lower=displayMin+b*binWidth,upper=lower+binWidth-1;bins.push({x:(lower+upper)/2,lower:lower,upper:upper,y:0,raw:0,outlierRaw:0});}
    rounded.forEach(function(point){var lowOutlier=stats.classified&&point.value<stats.lowerFence,highOutlier=stats.classified&&point.value>stats.upperFence;if(point.value<displayMin||(trimmed&&lowOutlier)){lowTail++;lowTailWeight+=point.w;return;}if(point.value>displayMax||(trimmed&&highOutlier)){highTail++;highTailWeight+=point.w;return;}var index=Math.floor((point.value-displayMin)/binWidth);index=Math.max(0,Math.min(binCount-1,index));weighted[index]+=point.w;raw[index]++;if(lowOutlier||highOutlier)outlierRaw[index]++;});
    for(var k=0;k<bins.length;k++){bins[k].y=weighted[k];bins[k].raw=raw[k];bins[k].outlierRaw=outlierRaw[k];}
    ...
    return{bins:bins,minimum:displayMin,maximum:displayMax,binWidth:binWidth,tickStep:tickStep,exact:binWidth===1,trimmed:trimmed,lowTail:lowTail,highTail:highTail,lowTailWeight:lowTailWeight,highTailWeight:highTailWeight};
  }
  function buildDiscrete(canvas,variable){
    ...
    if(!points.length||(CONFIG.weighted&&(!stats||stats.totalWeight<=0))){var noDataMessage=CONFIG.weighted&&points.length?tr("noPositiveWeight","No positive weight for this selection"):tr("noValidNumeric","No valid numeric values for this selection");setEmpty(canvas,true,noDataMessage);statistic(variable,"");clearSummary(variable,noDataMessage);describe(canvas,noDataMessage);return;}
    ...
    setEmpty(canvas,false);var statisticText=(CONFIG.weighted?tr("weightedMedian","weighted median"):tr("median","median"))+" "+fmt(stats.median,2)+" · "+(CONFIG.weighted?tr("weightedMean","weighted mean"):tr("mean","mean"))+" "+fmt(stats.mean,2)+" · "+tr("rawN","raw n")+"="+localNumber(stats.n)+(stats.classified?" · "+localNumber(stats.outliers)+" "+(stats.outliers===1?tr("outlier","Tukey outlier"):tr("outliers","Tukey outliers")):" · "+tr("outliersNotClassified","outliers not classified"));statistic(variable,statisticText);
    var nonzero=[];plan.bins.forEach(function(bin){if(bin.y>0){var label=plan.exact?axisNumber(bin.x,1):axisNumber(bin.lower,plan.binWidth)+"–"+axisNumber(bin.upper,plan.binWidth);nonzero.push(label+": "+fmt(bin.y,CONFIG.weighted?1:0)+" ("+tr("rawN","raw n")+"="+localNumber(bin.raw)+")");}});var tailBits=[];if(plan.lowTail)tailBits.push(localNumber(plan.lowTail)+" "+tr("belowDisplayRange","below the displayed range"));if(plan.highTail)tailBits.push(localNumber(plan.highTail)+" "+tr("aboveDisplayRange","above the displayed range"));var message=statisticText+"; "+tr("integerRange","integer range")+" "+plan.minimum+"–"+plan.maximum+(plan.exact?"":"; "+tr("integerBinWidth","integer bin width")+" "+plan.binWidth)+"; "+localNumber(zeroGaps)+" "+tr("zeroFrequencyValues",plan.exact?"zero-frequency values":"zero-frequency bins")+(tailBits.length?"; "+tailBits.join("; "):"")+"; "+nonzero.join("; ");clearSummary(variable,message);describe(canvas,message);canvas.dataset.discreteMin=""+plan.minimum;canvas.dataset.discreteMax=""+plan.maximum;canvas.dataset.discreteBinWidth=""+plan.binWidth;canvas.dataset.lowOutliers=""+plan.lowTail;canvas.dataset.highOutliers=""+plan.highTail;canvas.dataset.zeroFrequencyGaps=""+zeroGaps;
    var discreteColors=plan.bins.map(function(bin){return rgba(css(bin.outlierRaw>0?"--chart-coral":"--chart-purple",bin.outlierRaw>0?"#e5552b":"#7c5ba6"),.9);}),xPadding=plan.binWidth/2;
    state.charts[canvas.id]=new Chart(canvas,{type:"bar",plugins:[discreteMedianGuide],data:{datasets:[{data:plan.bins,parsing:{xAxisKey:"x",yAxisKey:"y"},backgroundColor:discreteColors,borderRadius:plan.binWidth===1?2:3,borderSkipped:false,categoryPercentage:.9,barPercentage:.96,maxBarThickness:plan.binWidth===1?(compact?14:20):(compact?28:36)}]},options:{maintainAspectRatio:false,layout:{padding:{top:22}},scales:{x:{type:"linear",min:plan.minimum-xPadding,max:plan.maximum+xPadding,offset:false,ticks:{stepSize:plan.tickStep,maxTicksLimit:compact?9:13,includeBounds:false,maxRotation:0,minRotation:0,precision:0,font:{size:9},callback:function(value){return axisNumber(value,1);}},grid:{display:false},border:{display:false},title:{display:!compact,text:plan.exact?tr("integerValue","Integer value"):tr("integerValueBins","Integer value (grouped)")}},y:{beginAtZero:true,ticks:{maxTicksLimit:compact?4:6,font:{size:9},precision:CONFIG.weighted?undefined:0},border:{display:false},title:{display:!compact,text:CONFIG.weighted?tr("weightedFrequency","weighted frequency"):tr("observations","observations")},grid:{color:css("--chart-grid","rgba(90,100,115,.13)")}}},plugins:{discreteMedianGuide:{median:stats.median,minimum:plan.minimum,maximum:plan.maximum,step:plan.tickStep,lowTail:plan.lowTail,highTail:plan.highTail},legend:{display:false},tooltip:{callbacks:{title:function(items){var datum=items[0].raw;return plan.exact?tr("value","Value")+" "+axisNumber(datum.x,1):tr("values","Values")+" "+axisNumber(datum.lower,plan.binWidth)+"–"+axisNumber(datum.upper,plan.binWidth);},label:function(ctx){var datum=ctx.raw,isOutlier=datum.outlierRaw>0;return " "+fmt(datum.y,CONFIG.weighted?1:0)+" "+(CONFIG.weighted?tr("weightedFrequency","weighted frequency"):tr("observations","observations"))+"; "+tr("rawN","raw n")+"="+localNumber(datum.raw)+(isOutlier?"; "+localNumber(datum.outlierRaw)+" "+(datum.outlierRaw===1?tr("tukeyOutlier","Tukey outlier"):tr("outliers","Tukey outliers")):"");},footer:function(){var tails=[];if(plan.lowTail)tails.push("◀ "+localNumber(plan.lowTail)+" "+tr("lowOutliers","low outliers"));if(plan.highTail)tails.push(localNumber(plan.highTail)+" "+tr("highOutliers","high outliers")+" ▶");return tails.join(" · ");}}}}}});
  }
  function buildHistogram(canvas,variable){
    ...
    if(!points.length||(CONFIG.weighted&&totalPointWeight<=0)){var noDataMessage=CONFIG.weighted&&points.length?tr("noPositiveWeight","No positive weight for this selection"):tr("noValidNumeric","No valid numeric values for this selection");renderNumericStats(variable,null);setEmpty(canvas,true,noDataMessage);statistic(variable,"");clearSummary(variable,noDataMessage);describe(canvas,noDataMessage);return;}
    var sorted=points.map(function(p){return p.value;}).sort(function(a,b){return a-b;});
    var med=weightedQuantile(points,.5),mean=weightedMean(points),q1=weightedQuantile(points,.25),q3=weightedQuantile(points,.75);
    var sd=weightedStd(points,mean,totalPointWeight),iqr=q3-q1,classified=points.length>=4,lowerFence=classified?q1-1.5*iqr:null,upperFence=classified?q3+1.5*iqr:null,lowOutliers=0,highOutliers=0,outlierWeight=0,inliers=[];
    points.forEach(function(point){if(classified&&point.value<lowerFence){lowOutliers++;outlierWeight+=point.w;}else if(classified&&point.value>upperFence){highOutliers++;outlierWeight+=point.w;}else inliers.push(point);});if(!inliers.length)inliers=points.slice();
    var inlierSorted=inliers.map(function(point){return point.value;}).sort(function(a,b){return a-b;}),lowerWhisker=inlierSorted[0],upperWhisker=inlierSorted[inlierSorted.length-1],low=lowerWhisker,high=upperWhisker;
    var stats={n:points.length,missing:Math.max(0,source.length-points.length),mean:mean,sd:sd,min:sorted[0],max:sorted[sorted.length-1],q1:q1,q3:q3,median:med,classified:classified,lowerFence:lowerFence,upperFence:upperFence,outliers:lowOutliers+highOutliers,totalWeight:totalPointWeight,outlierWeight:outlierWeight};renderNumericStats(variable,stats);
    if(points.length<2){var oneMessage=tr("oneValidValue","One valid value; open Stats for its summary");setEmpty(canvas,true,oneMessage);statistic(variable,tr("validN","valid n")+"=1 · "+tr("value","value")+" "+fmt(points[0].value,2));clearSummary(variable,oneMessage);describe(canvas,oneMessage);return;}
    setEmpty(canvas,false);if(high<=low){low=lowerWhisker-.5;high=upperWhisker+.5;}
    var span=high-low,width=iqr>0?2*iqr/Math.cbrt(inliers.length):span/Math.sqrt(inliers.length),minBins=compact?6:8,maxBins=compact?14:28,proposedBins=Math.max(minBins,Math.min(maxBins,Math.round(span/(width||1))));if(!isFinite(proposedBins)||proposedBins<1)proposedBins=compact?10:12;
    ...
    inliers.forEach(function(p){var index=Math.floor((p.value-alignedLow)/step);if(index>=bins)index=bins-1;if(index<0)index=0;counts[index]+=p.w;raw[index]++;});
    ...
    low=alignedLow;high=alignedHigh;
    var outlierCount=lowOutliers+highOutliers;
    var statisticText=(CONFIG.weighted?tr("weightedMedian","weighted median"):tr("median","median"))+" "+fmt(med,2)+" · "+(CONFIG.weighted?tr("weightedMean","weighted mean"):tr("mean","mean"))+" "+fmt(mean,2)+" · "+tr("rawN","raw n")+"="+localNumber(sorted.length)+(classified?" · "+localNumber(outlierCount)+" "+(outlierCount===1?tr("outlier","Tukey outlier"):tr("outliers","Tukey outliers")):" · "+tr("outliersNotClassified","outliers not classified"));
    statistic(variable,statisticText);clearSummary(variable,statisticText);describe(canvas,statisticText);
    var guide={q1:q1,q3:q3,median:med,lowerWhisker:lowerWhisker,upperWhisker:upperWhisker,displayLow:low,displayHigh:high,lowOutliers:lowOutliers,highOutliers:highOutliers},tickStep=niceNumericStep((high-low)/(compact?6:9),false,false);
    state.charts[canvas.id]=new Chart(canvas,{type:"bar",plugins:[distributionGuide],data:{datasets:[{data:histogramData,parsing:{xAxisKey:"x",yAxisKey:"y"},backgroundColor:rgba(css("--chart-purple","#7c5ba6"),.9),borderRadius:2,categoryPercentage:1,barPercentage:1}]},options:{maintainAspectRatio:false,layout:{padding:{top:25}},scales:{x:{type:"linear",min:low,max:high,offset:false,ticks:{stepSize:tickStep,maxTicksLimit:compact?7:10,includeBounds:false,maxRotation:0,minRotation:0,font:{size:9},callback:function(value){return axisNumber(value,tickStep);}},grid:{display:false},border:{display:false},title:{display:!compact,text:tr("valueWhisker","value (Tukey whisker range)")}},y:{beginAtZero:true,ticks:{maxTicksLimit:compact?4:6,font:{size:9},precision:0},border:{display:false},title:{display:!compact,text:CONFIG.weighted?tr("weightedFrequency","weighted frequency"):tr("observations","observations")},grid:{color:css("--chart-grid","rgba(90,100,115,.13)")}}},plugins:{distributionGuide:{stats:guide},legend:{display:false},tooltip:{callbacks:{title:function(items){var datum=items[0].raw;return axisNumber(datum.lower,step)+" "+tr("to","to")+" "+axisNumber(datum.upper,step);},label:function(ctx){return " "+fmt(ctx.raw.y,1)+" "+(CONFIG.weighted?tr("weighted","weighted"):tr("observations","observations"))+"; "+tr("rawN","raw n")+"="+localNumber(ctx.raw.raw);}}}}}});
  }
  ...
  function buildDate(canvas,variable){
    ...
    var keys=Object.keys(counts).sort();if(!keys.length||(CONFIG.weighted&&total<=0)){var emptyMessage=keys.length?tr("noPositiveWeight","No positive weight for this selection"):tr("noRecognizableDates","No recognizable dates for this selection");setEmpty(canvas,true,emptyMessage);statistic(variable,"");clearSummary(variable,emptyMessage);describe(canvas,emptyMessage);return;}
    ...
    state.charts[canvas.id]=new Chart(canvas,{type:"line",data:{labels:keys,datasets:[{data:keys.map(function(k){return counts[k];}),borderColor:trend,backgroundColor:trendFill,fill:true,tension:.22,pointRadius:compact?1.5:2,pointHoverRadius:4}]},options:{maintainAspectRatio:false,scales:{x:{grid:{display:false},border:{display:false},ticks:{maxTicksLimit:compact?6:10,font:{size:9}}},y:{beginAtZero:true,ticks:{maxTicksLimit:compact?4:6,font:{size:9},precision:0},border:{display:false},title:{display:!compact,text:CONFIG.weighted?tr("weightedFrequency","weighted frequency"):tr("observations","observations")},grid:{color:css("--chart-grid","rgba(90,100,115,.13)")}}},plugins:{legend:{display:false}}}});
  }
  ...
  function buildCompletion(canvas,variable){
    ...
    if(!source.length||(CONFIG.weighted&&aw+mw<=0)){var emptyMessage=source.length?tr("noPositiveWeight","No positive weight for this selection"):tr("noObservations","No observations for this selection");setEmpty(canvas,true,emptyMessage);statistic(variable,"");clearSummary(variable,emptyMessage);describe(canvas,emptyMessage);return;}setEmpty(canvas,false);
  ...
  function build(canvas){
    ...
    try{
      ...
      else if(kind==="discrete")buildDiscrete(canvas,variable);
  ...
  function highlight(variable,source){
    ...
    if(kind==="hist"){
      ...
      return numericMedian!==null?{value:fmt(numericMedian,2),detail:(CONFIG.weighted?tr("weightedMedian","Weighted median"):tr("median","Median"))+" · "+tr("validN","valid n")+"="+localNumber(points.length)}:{value:tr("notAvailable","n/a"),detail:points.length?tr("noPositiveWeight","No positive weight"):tr("noValidResponses","No valid responses")};
    }
    if(kind==="multibar"){
  ...
  function wireControls(){
    ...
    setControlsExpanded(false);updateControlSummary();
  }
  ...
  function wirePanelTabs(){
    document.querySelectorAll(".panel-tabs").forEach(function(tablist){var buttons=Array.prototype.slice.call(tablist.querySelectorAll('[role="tab"]')),panel=tablist.closest(".panel");
      function activate(button){var target=button.getAttribute("data-panel-view");buttons.forEach(function(item){var selected=item===button;item.setAttribute("aria-selected",selected?"true":"false");item.setAttribute("tabindex",selected?"0":"-1");});panel.querySelectorAll("[data-panel-view-pane]").forEach(function(pane){var active=pane.getAttribute("data-panel-view-pane")===target;pane.classList.toggle("is-active",active);pane.setAttribute("aria-hidden",active?"false":"true");});if(target==="distribution"){var canvas=panel.querySelector("canvas.chart");if(canvas&&state.charts[canvas.id])requestAnimationFrame(function(){state.charts[canvas.id].resize();});}scheduleChartVisibilityRefresh();}
  ...
  if(window.__SURVEYE_TEST_ONLY__){window.__SURVEYE_STATS__={quantile:quantile,weightedQuantile:weightedQuantile,weightedMean:weightedMean,weightedStd:weightedStd,weightedOutlierShare:weightedOutlierShare,effectiveN:effectiveN,confidenceInterval:confidenceInterval,categorical:categorical,numericValues:numericValues,numericStatistics:numericStatistics,niceNumericStep:niceNumericStep,discreteDistributionPlan:discreteDistributionPlan,monthKey:monthKey,isSpecial:isSpecial,isConfiguredMissing:isConfiguredMissing,filterMatches:filterMatches,labelFor:labelFor,colorFor:colorFor,stableColorIndex:stableColorIndex,balancedRows:balancedRows,groupLabel:groupLabel,horizontalScale:horizontalScale,barValuePlacement:barValuePlacement,chartIsGenuinelyVisible:chartIsGenuinelyVisible,buildSplit:buildSplit,buildFamily:buildFamily,buildDiscrete:buildDiscrete,buildHistogram:buildHistogram,buildComparison:buildComparison,panelKey:panelKey};return;}
