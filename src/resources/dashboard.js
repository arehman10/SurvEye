(function(){
  ...
  function exportColumns(){
    ...
    var columns=[];Object.keys(DATA[0]).forEach(function(key){if(key.charAt(0)!=="_")columns.push(key);});
    return columns;
  }
  ...
  function exportChartImage(panel){
    ...
    ensureBuilt(canvas);
    var titleNode=panel.querySelector(".panel-title"),titleText=titleNode?(""+titleNode.textContent).trim():"";
    try{
      var ratio=window.devicePixelRatio||1,pad=Math.round(16*ratio),head=titleText?Math.round(30*ratio):0,
          width=(canvas.width||1)+pad*2,height=(canvas.height||1)+pad*2+head,
          target=document.createElement("canvas");
      target.width=width;target.height=height;
      var context=target.getContext("2d");
      context.fillStyle=css("--card","#ffffff");context.fillRect(0,0,width,height);
      if(titleText){
        context.fillStyle=css("--ink","#16202e");
        context.font="600 "+Math.round(13*ratio)+"px \"Public Sans\",Inter,\"Segoe UI\",Arial,sans-serif";
        context.textBaseline="middle";context.textAlign=rtl?"right":"left";
        context.fillText(titleText,rtl?width-pad:pad,pad+Math.round(head/2)-Math.round(3*ratio));
      }
      context.drawImage(canvas,pad,pad+head);
      downloadDataUri(target.toDataURL("image/png"),slugify(panelKey(canvas))+".png");
    }catch(ignore){}
  }
