...
```stata
net install surveye, from("C:/path/to/release/surveye-2.1.3-ssc") replace
discard
which surveye
help surveye
```
...
- `surveye-2.1.3.zip` — full source, tests, documentation, example, and JARs;
- `surveye-2.1.3-github.zip` — the same clean source with its contents flat at
  the archive root, ready to upload to the GitHub repository root;
- `surveye-2.1.3-ssc.zip` — only the flat installable package; and
...
- Upload the *contents* of `surveye-2.1.3-github.zip` to the root of the
...
- Create an annotated `v2.1.3` tag and GitHub release.
