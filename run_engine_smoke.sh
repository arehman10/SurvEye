...
async function testStatsTabs(page) {
  ...
  const details = await page.evaluate(() => Array.from(document.querySelectorAll('[data-panel-view="stats"]'), (button) => {
    ...
    const outlierField = !table || (finalRow?.querySelectorAll("th,td").length >= 4 &&
  ...
  const failed = details.filter((item) => !item.selected || !item.paneVisible || !item.distributionHidden ||
    !item.populated || !item.outlierField || !item.restored).map((item) => item.variable || "unnamed");
...
async function checkOverflow(page) {
  ...
  await page.evaluate((states) => {
    document.querySelectorAll("details").forEach((element, index) => { element.open = Boolean(states[index]); });
  }, originalDetails);
  await settle(page, 60);
  return result;
}
...
function checkFailures(viewportReport) {
  ...
  for (const name of ["motion", "search", "filter", "details", "map", "charts", "stats"]) {
