...
| Set language and appearance | `uilanguage()`, `direction()`, `title()`, `subtitle()`, `logo()`, `theme()`, `density()`, `background()`, `typography()`, `corners()`, `shadow()`, `motion()`, `pagewidth()` |
...
SurvEye now defaults to `theme(editorial)`.  This carries over the strongest
visual choices from the generic informality dashboard without carrying over any
survey-specific variables or text: a warm paper canvas, restrained cyan and
gold glow, World Bank role colors, an editorial serif hierarchy, softened card
edges, shallow shadows, and reduced-motion-safe entrance transitions.  The HTML
remains self-contained; the finish uses bundled and system font stacks rather
than fetching Google Fonts.

Use the preset alone or override individual layers:

```stata
surveye using "questionnaire.html", saving("editorial.html") ///
    theme(editorial) replace open

surveye using "questionnaire.html", saving("custom_finish.html") ///
    theme(editorial) background(paper) typography(modern) ///
    corners(soft) shadow(lifted) motion(none) pagewidth(1440) ///
    replace open
```

`background()` accepts `auto`, `glow`, `paper`, or `plain`; `typography()` accepts
`auto`, `editorial`, `modern`, or `system`; `corners()` accepts `auto`, `rounded`,
`soft`, or `square`; `shadow()` accepts `auto`, `soft`, `lifted`, or `none`; and
`motion()` accepts `none`, `subtle`, or `reveal`.  `pagewidth()` accepts 0 or a
maximum content width from 960 to 2,000 pixels.  `auto` leaves that layer to the
selected legacy theme.  Explicit `theme(worldbank)`, `theme(clean)`,
`theme(forest)`, and `theme(dark)` continue to use their earlier appearance
unless one of the new layer options is supplied.
